...
public class JsonIO {
    ...
    /*
    public static JSONObject pushJson(String url, String jsonDataName, JSONObject json) {
        Map<String, byte[]> map = new HashMap<String, byte[]>();
        map.put(jsonDataName, UTF8.getBytes(json.toString()));
        StringBuilder sb = pushMap(url, map);
        try {
            json = new JSONObject(sb.toString());
            return json;
        } catch (JSONException e) {
    ...
    public static StringBuilder pushMap(String url, Map<String, byte[]> post) {
        ...
        try {
            ...
            try {
                ...
                for (Map.Entry<String, byte[]> entry: post.entrySet()) {
                    out.writeBytes(boundary + crlf);out.flush();
                    out.writeBytes("Content-Disposition: form-data; name=\"" +entry.getKey() + "\"" + crlf);out.flush();
                    out.writeBytes(crlf);out.flush();
                    out.write(entry.getValue());out.flush();
                    out.writeBytes(crlf);out.flush();
                }
