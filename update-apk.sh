...
# Clone the repo on gh-pages branch after cding to $HOME
cd $HOME
...
# Force update everything and add everything
cp -rf $HOME/work_new/ ./
git add .
...
if [ "$?" = 0 ]; then
  git commit --message "Apk update for build:$TRAVIS_BUILD_NUMBER | Generated by Travis CI"
